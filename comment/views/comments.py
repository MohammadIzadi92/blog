    ...
    def form_valid(self, form):
        ...
        if self.comment.parent:
            ...
            if parent_email in [author_email , user_email]:
                parent_email = False           
        if author_email:
            email = EmailMessage('دیدگاه جدید','دیدگاه جدیدی برای مقاله < {} > که شما نویسنده آن هستید ارسال شده :\n{}{}'.format(
                article,current_site,reverse('blog:detail_blog',kwargs={'slug':article.slug})),to=[author_email])
            email.send()
        if parent_email:
            email = EmailMessage('دیدگاه جدید','پاسخی به دیدگاه شما در مقاله {} داده شده است :\n {}{}'.format(
                article,current_site,reverse('blog:detail_blog',kwargs={'slug':article.slug})),to=[parent_email])
            email.send()
        if user_email:
            email = EmailMessage('دیدگاه جدید','دیدگاه شما با موفقیت ثبت شد',to=[user_email])
            email.send()
        # end send email
